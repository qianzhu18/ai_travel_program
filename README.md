...
为方便在 GitLens 里快速看懂历史，约定如下：

1. 所有提交信息使用中文（可带英文前缀，如 `fix:`、`chore:`）。
2. 修改前先给出中文改动说明，再执行代码变更。
3. 每次修复完成后，额外补一个中文“备份提交”可选（用于验收节点留痕）。

推荐提交格式：
```
fix: 修复模板图片上传鉴权导致的 403
chore: 解决图片上传到模板库的问题（备份）
```
